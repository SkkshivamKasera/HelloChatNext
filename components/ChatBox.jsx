...
const ChatBox = ({ chat, index, currentUser, currentChatId }) => {
    ...
    return (
        <div className={`${chat._id === currentChatId ? "bg-blue-2" : ""} chat-box`} onClick={() => router.push(`/chats/${chat?._id}`)}>
            <div className="chat-info">
                {
                    chat?.isGroup ? (
                        <img src={chat.groupPhoto || "/assets/group.png"} alt='group-photo' className='profilePhoto' />
                    ) : (
                        <img src={otherMember[0].profileImage || "/assets/person.jpg"} alt='profile-photo' className='profilePhoto' />
                    )
                }
                <div className='flex flex-col gap-1'>
                    ...
                    {
                        !lastMessage && (
                            <p className='text-small-bold'>Start a conversation</p>
                        )
                    }
                    {
                        lastMessage?.photo ? (
